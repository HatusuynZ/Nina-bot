...
export async function runCommand(command, ctx) {
  if (command.guildOnly !== false && !ctx.guild) {
    await ctx.replyPrivate('This command only works inside a server.').catch(() => {});
    return;
  }

  if (command.permission && !ctx.member?.permissions.has(command.permission)) {
    await ctx.replyPrivate("You don't have permission to use this command.").catch(() => {});
    return;
  }
  ...
  try {
    ...
    await ctx.replyPrivate('Something went wrong running that command.').catch(() => {});
  }
}
