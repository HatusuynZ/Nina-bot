...
class Context {
  ...
  getRole(name) {
    if (this.interaction) return this.interaction.options.getRole(name) ?? null;
    // texto: primeira mencao de cargo na mensagem
    return this.message.mentions.roles.first() ?? null;
  }

  getChannel(name) {
    if (this.interaction) return this.interaction.options.getChannel(name) ?? null;
